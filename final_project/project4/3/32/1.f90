program main
      implicit none
      real(8) :: lattice(32,32),jex1,jex2,b,randomx,randomy,h1,h2,tmp,t,summationh,summationm,averageh,averagem,mag,start,finish
	real(8) :: m2_avg,m4_avg,e,m,m2,m4,u
...
!      print*,lattice
	...
	!subroutine,处理得到磁矩？h1（哈密顿量）？对初始化并翻转后的lattice计算此时的性质（磁性，哈密顿量）
      call hamil(grid,jex1,jex2,b,lattice,mag,h1)
      summationh=h1
!      mag=abs(mag)
      summationm=mag
      cont=1
...
!      write(k,*) "steps:",cont,"Hamiltonian:",h1,"Magnetization:",mag
	...
	!随机翻转一个格点上的磁矩，并计算磁化强度，哈密顿量，循环指定步数
      ...
      call hamil(grid,jex1,jex2,b,lattice,mag,h2)
!      mag=abs(mag)
...
real function exchange1(s0,su,sd,sl,sr)
        implicit none
        integer :: s0,su,sd,sl,sr
        exchange1=s0*(su+sd+sl+sr)
end function
...
subroutine hamil(grid,jex1,jex2,b,lattice,mag,h)
        implicit none
        real(8) :: lattice(32,32),h,jex1,jex2,b,summation1,summation2,mag        !这里x没有问题？
        integer :: i,j,s0,su,sd,sl,sr,grid,sul,sur,sdl,sdr
        real,external :: exchange1, exchange2, efield
	...
	!计算第一项，交换关联
        ...
        summation1=summation1+exchange1(s0,su,sd,sl,sr)
	...
	summation2=summation2/2
        h=-jex1*summation1-jex2*summation2-b*mag
